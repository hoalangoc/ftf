...
<div style="display:none; text-align:center; background:url(application/modules/Contactimporter/externals/images/loading.gif) no-repeat;width:320px;height:320px; padding-top:180px;" id="loading">    
...
<div id="openinviter" style="width: 70%">
	<div>
		...
		<p class="description"><?php echo $this -> translate('The following people are not your friends yet, please select to add connections.'); ?></p>
		<br />
		...
		<form method="post" action="<?php echo $this -> url(array(
				'action' => 'add'
			), 'contactimporter_general', true);?>" class="global_form" name='openinviter' enctype="application/x-www-form-urlencoded" onsubmit="return check_select();">
		<td>
		<input type="hidden" value="<?php echo $this->invite_list?>" name="invite_list" />
		...
		<button type='submit' id='submit' name='send' ><?php echo $this -> translate('Add Connection'); ?> (<span id = "count_contacts">0</span>)</button> 
		</td>
		<td>
		&nbsp;&nbsp;<button type='submit' id='submit2' name='skip' onclick="skipSendInvite();return false;"><?php echo $this -> translate('Skip'); ?> &gt;&gt;</button>     
		</form>
		<form  method="post" action="<?php echo $this -> url(array(
				'action' => 'add'
			), 'contactimporter_general', true);
		?>" id = "skip"> 
