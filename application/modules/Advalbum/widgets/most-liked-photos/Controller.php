<?php
class Advalbum_Widget_MostLikedPhotosController extends Engine_Content_Widget_Abstract
{
	public function indexAction()
	{
		...
		if (!$is_ajax)
		{
			...
			$select = $table -> select() -> from($Name) -> joinLeft($aName, "$aName.album_id = $Name.album_id", '') -> where("$aName.search = ?", "1") -> order("$Name.like_count DESC");
