<?php
class Advalbum_Widget_ThisMonthPhotosController extends Engine_Content_Widget_Abstract {
	public function indexAction() {
		...
		if (! $is_ajax) {
			...
			$select = $table->select ()
			->from ( $Name )
			->joinLeft ( $aName, "$Name.album_id = $aName.album_id", '' )
			->where ( "search = ?", "1" )
			->where ( "YEAR($Name.creation_date) = YEAR(NOW())" )
			->where ( "MONTH($Name.creation_date) = MONTH(NOW())" )
			->order("$Name.view_count DESC")->limit($limit);
