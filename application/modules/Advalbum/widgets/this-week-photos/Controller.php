<?php
class Advalbum_Widget_ThisWeekPhotosController extends Engine_Content_Widget_Abstract {
	public function indexAction() {
		...
		if (! $is_ajax) {
			...
			->where ( "search = ?", "1" )->where ( "YEAR($Name.creation_date) = YEAR(NOW())" )
			->where ( "WEEKOFYEAR($Name.creation_date) = WEEKOFYEAR(NOW())" )
